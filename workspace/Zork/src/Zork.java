...
public class Zork {	
	...
	public static void visitRoom2() {
		...
		int response = sc.nextInt();
		if (response == 1) {
			pathFinder += "You entered the Foyer and saw dead scorpions. ";
			System.out.println("Your total money now is: $" + totalMoney);
			checkForBurglar(1);
			visitRoom1();
		}
		if (response == 2) {
			pathFinder += "You entered the Library and saw spiders.";
			payMoney(3);
			checkForBurglar(3);
			visitRoom3();
		} else {
			pathFinder += "You entered the Kitchen and saw bats. ";
			payMoney(4);
			checkForBurglar(4);
			visitRoom4();
		}
	...
	public static void visitRoom4() {
		...
		if (response == 1) {
			...
			pathFinder += "You entered the Parlor and saw a treasure chest. ";
			payMoney(7);
			checkForBurglar(7);
			visitRoom7();
		}
	...
	public static void visitRoom6() {
		...
		if (unlocked = false) {
			if (response == 1) {
				...
				int randomInt = 1 + rnd.nextInt(4);
				if (randomInt == 1) {
					pathFinder += "You unlocked and entered the Secret Room and see piles of gold. ";
					payMoney(8);
					checkForBurglar(8);
					visitRoom8();
				} else {
					pathFinder += "You entered the Parlor and see a treasure chest. ";
					payMoney(7);
					checkForBurglar(7);
					visitRoom7();
				}
			...
			pathFinder += "You entered the Secret Room and see piles of gold. ";
			payMoney(8);
			checkForBurglar(8);
			visitRoom8();
		}
	...
	public static void visitRoom7() {
		...
		int response = sc.nextInt();
		if (response == 1) {
			pathFinder += "You entered the Vault and see 3 walking skeletons. ";
			payMoney(6);
			checkForBurglar(6);
			visitRoom6();
		} else {
			pathFinder += "You entered the Kitchen and see bats. ";
			payMoney(4);
			checkForBurglar(4);
			visitRoom4();
		}
